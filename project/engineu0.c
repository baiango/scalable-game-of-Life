...
static const i32 ONE_SECOND = 1000;
static void _OprtTps(GameEvent *Evt)
{	u64 currentTicks = SDL_GetTicks64();
	
	u64 *totalFrame = &Evt->totalFrame;
	u64 *lastRecordedframe = &Evt->lastRecordedframe;
	u64 *prtTpsNextTimeout = &Evt->prtTpsNextTimeout;
	
	if (currentTicks > *prtTpsNextTimeout)
	{	printf("Ticks: %llu\n", *totalFrame - *lastRecordedframe);
		*prtTpsNextTimeout = currentTicks + ONE_SECOND,
		*lastRecordedframe = *totalFrame; }
...
{	// % is modulo.
	...
	// "((y-1) * row + x-1 + arrSize) % arrSize".
	// or y * row + x just for array index.
	u8 ret = 0;
	const u32 oneDPosY = y * row;
	const u32 mov1Y = 1 * row;
	const u32 arrSize = row * col;
	// top left
	ret += state[(oneDPosY-mov1Y + x-1 + arrSize) % arrSize];
	ret += state[(oneDPosY-mov1Y + x   + arrSize) % arrSize];
	ret += state[(oneDPosY-mov1Y + x+1 + arrSize) % arrSize];
	// left
	ret += state[(oneDPosY       + x-1 + arrSize) % arrSize];
	ret += state[(oneDPosY       + x+1 + arrSize) % arrSize];
	// bottom left
	ret += state[(oneDPosY+mov1Y + x-1 + arrSize) % arrSize];
	ret += state[(oneDPosY+mov1Y + x   + arrSize) % arrSize];
...
static void OprocessKeyDown(GameEvent *Evt, u0 *state, GameDefine *Gdef)
...
{	while (SDL_PollEvent(&Evt->Event))
	{	switch (Evt->Event.type)
		{case SDL_QUIT:
			Evt->quit = true;
		case SDL_KEYDOWN:
			OprocessKeyDown(Evt, state, Gdef); }}}
